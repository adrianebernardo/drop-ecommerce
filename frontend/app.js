...
async function loadProducts() {
  ...
  try {
    ...
    if (!products.length) {
      grid.innerHTML = '<p class="loading">Nenhum produto ainda.</p>';
      return;
    }
    ...
    grid.innerHTML = products.map(p => `
      <div class="card" onclick="openModal(${JSON.stringify(p).replace(/"/g, '&quot;')})">
        <div class="card-image">
          ${
            p.image_url
              ...
              : `<div class="card-img-placeholder">DRØP</div>`
          }
        </div>
        ...
        <div class="card-info">
          ...
          <button
            class="card-add"
            onclick="event.stopPropagation(); openModal(${JSON.stringify(p).replace(/"/g, '&quot;')})"
          >
            ADD TO BAG
          </button>
        </div>
...
function addToCart(p, size) {
  ...
  if (existing) {
    ...
    cart.push({
      ...p,
      image: p.image_url || p.image_product || '',
      qty: 1,
      quantity: 1,
      size,
      _key: key
...
function updateCart() {
  ...
  document.getElementById('cartItems').innerHTML =
    cart.length
      ...
      ? cart.map(i => `
          <div class="cart-item">
            ...
            <div class="cart-item-controls">
              ...
              <button onclick="changeQty('${i._key}', -1)">
                −
              </button>
      ...
      : '<p style="color:#999;font-size:.85rem;letter-spacing:2px;">Seu bag está vazio.</p>';
}
...
function changeQty(key, delta) {
  ...
  // Impede ultrapassar o estoque disponível
...
// SINCRONIZA UI COM CARRINHO SALVO
updateCart();
